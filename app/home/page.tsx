  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: profile }, { data: novels }] = await Promise.all([
        ...
        supabase.from("novels").select("*, chapters(id, num, title, free, published_at, vote_open, vote_closed, winner_option_id, vote_options(id, text, votes), comments(id, text, likes, created_at, user_id, profiles(name, avatar, role)))").order("created_at")
      ])
